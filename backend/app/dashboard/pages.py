...
from app.dashboard.badges import build_outcomes
...
from app.dashboard.assignment_time import add_assignment_time
from app.dashboard.course_timing import add_engagement_stages, add_badge_delays
...
def make_page(name, data, filters):
    ...
    if name == "overview":
        ...
        for key, students in (("assignment_students", a), ("engagement_students", e), ("intersection_students", a & e)):
            required = {"assignment", "engagement"} if key.startswith("intersection") else {"assignment" if key.startswith("assignment") else "engagement"}
        ...
        build_outcomes(page, data, scope, reason)
    elif name == "insights":
    ...
    required = {"engagement": {"engagement"}, "assignments": {"assignment"},
                "outcomes": {"badge", "engagement"}}.get(name, set())
    ...
    if name in {"overview", "engagement", "assignments", "outcomes"}:
        ...
        if name in {"overview", "assignments", "outcomes"} and "assignment" in data.batches:
        ...
        if name in {"overview", "assignments", "outcomes"}:
            add_survey(page, data, scope, assessment_only=name == "assignments")
        ...
        if name == "outcomes" and {"badge", "engagement"} <= data.batches.keys():
            add_badge_delays(page, data, scope, reason)
    return page
